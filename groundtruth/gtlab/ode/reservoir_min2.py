...
States: level L (volume), quality Q, return flow R, clock c (ticks since reset).
    dL/dt = inflow(c) + R - delivered(L, u) - spill(L) - seep * L
    dQ/dt = (q_base - Q) / tau_q
    ...
    spill(L)     = K_SPILL * softplus(L - l_cap)                soft full-pool spill, K_SPILL = 1 fixed
...
STATE_HI = np.array([2000.0, 1.0, 5.0, 1e9])
...
def _flows(M, L, uu, th):
    rel, irr, dep, aer = uu
    cap = th["c_out"] * M.pow(L / _LREF, th["p_out"])
    delivered = _smin(M, rel + irr, cap, 0.5)
    spill = _K_SPILL * _pos(M, L - th["l_cap"], 5.0)
    return delivered, spill
...
def f(x, u, th, mech):
    ...
    L, Q, R, c = s
    irr = uu[1]
    delivered, spill = _flows(M, L, uu, th)
    dL = _inflow(M, c, th) + R - delivered - spill - th["seep"] * L
    dQ = (th["q_base"] - Q) / th["tau_q"]
...
def h(x, u, th, mech):
    ...
    L, Q, R, c = s
    delivered, spill = _flows(M, L, uu, th)
    return _pack(M, [L, _inflow(M, c, th) + R, delivered + spill, Q])
