...
def _ode_ad_auction_min():
    import math
    import numpy as np
    FAMILY = 'ad_auction_min'
    OBS = ['win_rate', 'spend', 'conversions']
    CTRL = ['bid', 'budget_cap', 'targeting_breadth']
    MECHS = {'A': 'exposure removes reachable people (always on)', 'B': 'fulfilment queue with limited capacity (always on)', 'C': 'unused'}
    N_SUB = 2
    STATE = ['X', 'Q1', 'Q2']
    _NS = len(STATE)
    STATE_LO = np.zeros(_NS)
    STATE_HI = np.array([1.0, 1000000.0, 1000000.0])
    PARAMS = [('N', 255.0, 20.0, 5000.0, True), ('b0', 3.0, 0.2, 30.0, True), ('nw', 1.5, 0.3, 4.0, False), ('gam', 0.35, 0.0, 1.0, False), ('kd', 0.1, 0.005, 2.0, True), ('tau_e', 40.0, 3.0, 400.0, True), ('c', 0.25, 0.01, 2.0, True), ('tau1', 12.0, 0.7, 60.0, True), ('tau2', 8.0, 0.7, 60.0, True), ('F', 6.0, 0.5, 100.0, True), ('kb', 1.0, 0.0, 10.0, False), ('kw', 0.3, 0.0, 3.0, False)]
    ...
    _obj_q7.__dict__.update(locals())
    return _obj_q7
_ODE_BUILD = {'ad_auction_min': _ode_ad_auction_min}
_ODE_MODS = {}
