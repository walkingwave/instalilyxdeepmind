...
def _ode_wildlife_min():
    import math
    import numpy as np
    FAMILY = 'wildlife_min'
    OBS = ['prey_north', 'predator_north', 'prey_south', 'predator_south']
    CTRL = ['hunting_quota', 'habitat_protection', 'corridor_access']
    MECHS = {'A': 'finite food stock (always on)', 'B': 'sheltered hunting exposure (always on)', 'C': 'unused'}
    N_SUB = 2
    STATE = ['PN', 'LN', 'QN', 'PS', 'LS', 'QS']
    _NS = len(STATE)
    STATE_LO = np.zeros(_NS)
    STATE_HI = np.array([10000.0, 10000.0, 1000.0, 10000.0, 10000.0, 1000.0])
    PARAMS = [('r', 0.25, 0.05, 1.5, True), ('c', 53.0, 3.0, 500.0, True), ('dl', 0.00064, 1e-05, 0.1, True), ('rho', 0.038, 0.005, 0.5, True), ('hq', 0.026, 0.001, 1.0, True), ('ks', 0.83, 0.3, 1.5, False), ('kh', 0.52, 0.05, 1.0, True), ('en', 0.5, 0.0, 0.95, False), ('s', 0.064, 0.003, 0.5, True), ('mc', 0.019, 0.0, 0.3, False), ('q0', 2.45, 0.1, 20.0, True), ('qc', 0.76, 0.0, 3.0, False)]
    ...
    _obj_q7.__dict__.update(locals())
    return _obj_q7
_ODE_BUILD = {'wildlife_min': _ode_wildlife_min}
_ODE_MODS = {}
