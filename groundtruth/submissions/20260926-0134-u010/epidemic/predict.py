...
def _ode_epidemic():
    ...
    MECHS = {'A': 'behavioural fatigue: compliance decays with cumulative restriction', 'B': 'developing/waning immunity: R returns to S', 'C': 'postponed gatherings: restriction builds a debt that raises contacts on release'}
    N_SUB = 2
    NAGE = np.array([0.2, 0.55, 0.25])
    CBASE = [[3.0, 1.5, 0.5], [1.5, 2.0, 0.6], [0.5, 0.6, 1.0]]
    EMIX = [0.3, 0.55, 0.15]
    STATE = [f'S{i}' for i in range(3)] + [f'E{i}' for i in range(3)] + [f'I{i}' for i in range(3)] + [f'R{i}' for i in range(3)] + ['H', 'WL', 'fatigue', 'debt']
    _NS = len(STATE)
    STATE_LO = np.zeros(_NS)
    STATE_HI = np.full(_NS, 1.0)
    STATE_HI[12] = 10000000.0
    STATE_HI[13] = 10000000.0
    STATE_HI[15] = 10000.0
    PARAMS = [('pop', 1000000.0, 10000.0, 1000000000.0, True), ('beta', 0.07, 0.003, 2.0, True), ('sigma', 0.25, 0.05, 1.5, True), ('gam_c', 0.2, 0.02, 1.5, True), ('gam_a', 0.15, 0.02, 1.5, True), ('gam_e', 0.1, 0.02, 1.5, True), ('sev_c', 0.002, 1e-05, 0.5, True), ('sev_a', 0.01, 1e-05, 0.5, True), ('sev_e', 0.06, 1e-05, 0.8, True), ('los', 8.0, 1.0, 200.0, True), ('hcap', 500.0, 5.0, 1000000.0, True), ('tau_wl', 2.0, 0.5, 50.0, True), ('wl_leave', 0.1, 0.0, 1.0, False), ('school', 0.6, 0.0, 0.95, False), ('close_eff', 0.8, 0.0, 1.0, False), ('mask_eff', 0.4, 0.0, 0.9, False), ('k_clinic', 1.0, 0.0, 20.0, False), ('imm0', 0.3, 0.0, 0.95, False), ('tau_fat', 200.0, 10.0, 5000.0, True), ('fat_k', 0.5, 0.0, 0.95, False), ('tau_wane', 400.0, 20.0, 10000.0, True), ('debt_k', 0.5, 0.0, 5.0, False), ('tau_debt', 30.0, 2.0, 1000.0, True)]
    ...
    def x0(y0, th, mech):
        ...
        S = [_pos(M, 1.0 - E[i] - I[i] - R[i], 0.0001) for i in range(3)]
        z = 0.0 * cases
    ...
    _obj_q7.__dict__.update(locals())
    return _obj_q7
_ODE_BUILD = {'epidemic': _ode_epidemic}
_ODE_MODS = {}
...
SYSTEM = 'epidemic'
HARD = {'daily_cases': [0.0, None], 'hospital_load': [0.0, None]}
