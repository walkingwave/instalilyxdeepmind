...
def _ode_reservoir_min():
    import math
    import numpy as np
    FAMILY = 'reservoir_min'
    OBS = ['level', 'inflow', 'outflow', 'quality']
    CTRL = ['release_rate', 'irrigation_allocation', 'withdrawal_depth', 'aeration']
    MECHS = {'A': 'head-limited discharge (always on)', 'B': 'seepage + soft spill (always on)', 'C': 'unused'}
    N_SUB = 2
    STATE = ['L', 'Q', 'R', 'c']
    _NS = len(STATE)
    STATE_LO = np.zeros(_NS)
    STATE_HI = np.array([941.3, 1.0, 5.0, 1000000000.0])
    _LREF = 500.0
    _L_FULL = 941.3
    _TWO_PI = 2.0 * math.pi
    PARAMS = [('q_m', 11.43, 8.0, 15.0, False), ('q_a', 2.17, 0.5, 5.0, False), ('per', 67.8, 40.0, 110.0, True), ('phi', 0.04, -1.6, 1.6, False), ('seep', 0.0014, 0.0001, 0.02, True), ('c_out', 13.3, 5.0, 30.0, True), ('p_out', 0.33, 0.05, 1.0, False), ('q_base', 0.935, 0.5, 1.0, False), ('q_aer', 0.01, 0.0, 0.05, False), ('tau_q', 3.0, 0.67, 50.0, True), ('k_ret', 0.05, 0.0, 0.3, False), ('tau_ret', 40.0, 5.0, 50.0, True)]
    ...
    _obj_q7.__dict__.update(locals())
    return _obj_q7
_ODE_BUILD = {'reservoir_min': _ode_reservoir_min}
_ODE_MODS = {}
...
SYSTEM = 'reservoir'
HARD = {'level': [0.0, None], 'inflow': [0.0, None], 'outflow': [0.0, None], 'quality': [0.0, 1.0]}
