...
def _ode_epidemic_sirs():
    import math
    import numpy as np
    FAMILY = 'epidemic_sirs'
    OBS = ['daily_cases', 'hospital_load']
    CTRL = ['school_closure', 'mask_mandate', 'vaccination_rate']
    MECHS = {'A': 'waning immunity (always on)', 'B': 'hospital pressure on clinics (always on)', 'C': 'unused'}
    N_SUB = 2
    STATE = ['S', 'E', 'I', 'R', 'H']
    _NS = len(STATE)
    STATE_LO = np.zeros(_NS)
    STATE_HI = np.array([1.0, 1.0, 1.0, 1.0, 1000000.0])
    PARAMS = [('pop', 200000.0, 1000.0, 100000000.0, True), ('beta', 0.5, 0.02, 5.0, True), ('sigma', 0.3, 0.05, 2.0, True), ('gamma', 0.2, 0.02, 1.5, True), ('tau_wane', 150.0, 10.0, 5000.0, True), ('close_eff', 0.4, 0.0, 0.95, False), ('mask_eff', 0.4, 0.0, 0.95, False), ('vac_eff', 1.0, 0.0, 30.0, False), ('k_clinic', 1.0, 0.0, 20.0, False), ('h_ref', 100.0, 1.0, 100000.0, True), ('hfrac', 0.01, 1e-05, 0.9, True), ('los', 20.0, 1.0, 300.0, True), ('imm0', 0.3, 0.0, 0.95, False)]
    ...
    _obj_q7.__dict__.update(locals())
    return _obj_q7
_ODE_BUILD = {'epidemic_sirs': _ode_epidemic_sirs}
_ODE_MODS = {}
