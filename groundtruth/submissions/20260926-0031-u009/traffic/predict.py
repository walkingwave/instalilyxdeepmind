...
def _ode_traffic_min():
    import math
    import numpy as np
    FAMILY = 'traffic_min'
    OBS = ['flow_a', 'flow_b', 'speed_a', 'speed_b']
    CTRL = ['signal_timing', 'lane_closure', 'toll', 'ramp_metering', 'freight_priority', 'clearance_effort']
    MECHS = {'A': 'capacity-limited junction and exits (always on)', 'B': 'occupancy-dependent speed, finite approach buffer (always on)', 'C': 'unused'}
    N_SUB = 2
    STATE = ['p1a', 'p2a', 'p3a', 'p1b', 'p2b', 'p3b', 'sa', 'sb']
    _NS = len(STATE)
    STATE_LO = np.zeros(_NS)
    STATE_HI = np.array([100000.0, 100000.0, 100000.0, 100000.0, 100000.0, 100000.0, 200.0, 200.0])
    PARAMS = [('dem0', 40.0, 5.0, 300.0, True), ('w_sig', 1.0, 0.0, 1.5, False), ('r_pipe', 0.25, 0.05, 1.5, True), ('junc', 40.0, 3.0, 600.0, True), ('cap', 15.0, 3.0, 300.0, True), ('lc_a', 0.3, 0.0, 1.0, False), ('lc_b', 0.7, 0.0, 1.0, False), ('clr', 0.8, 0.0, 3.0, False), ('v_free', 48.9, 30.0, 70.0, False), ('n_ref', 200.0, 10.0, 5000.0, True), ('rho', 0.25, 0.02, 1.5, True), ('n_max', 800.0, 50.0, 20000.0, True)]
    ...
    def _exits(M, s, uu, th):
        ...
        cap_a = th['cap'] * (1.0 - th['lc_a'] * lane) * boost + 1e-06
        cap_b = th['cap'] * (1.0 - th['lc_b'] * lane) * boost + 1e-06
    ...
    def f(x, u, th, mech):
        ...
        j_a = _smin(M, r * p1a, th['junc'] * share + 1e-06)
        j_b = _smin(M, r * p1b, th['junc'] * (1.0 - share) + 1e-06)
    ...
    _obj_q7.__dict__.update(locals())
    return _obj_q7
_ODE_BUILD = {'traffic_min': _ode_traffic_min}
_ODE_MODS = {}
