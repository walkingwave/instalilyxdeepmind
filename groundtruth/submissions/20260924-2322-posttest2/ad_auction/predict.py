...
def _roll_l1(blob, doc, y0, U, ctx):
    ...
    c = _arr(blob['c'])
    K = a.shape[0]
    Q = np.einsum('tf,kfp->tkp', F, W)
    v0 = g_fwd(y0, tr)
    z = np.einsum('kpq,q->kp', E, v0) + b
    hist = blob.get('hist')
    T = Q.shape[0]
    if hist:
        ah = _arr(hist['ah'])
        gam = _arr(hist['gamma'])
        d = np.sqrt(np.mean((norm_u(doc, U) - _arr(hist['urec'])) ** 2, axis=1))
    oma = 1.0 - a
...
def rollout_from_blob(blob, y0, U, doc=None, base_dir=None, tag='', clip=True):
    ...
    return apply_post(doc, Y, U)

def apply_post(doc, Y, U):
    ...
    for r in rules:
        if r.get('type') == 'le_control' and r.get('obs') in obs and (r.get('control') in ctrls):
            ...
            Y[:, j] = np.minimum(Y[:, j], U[:, k] * float(r.get('scale', 1.0)))
    return Y
